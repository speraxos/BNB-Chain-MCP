...
export function registerGeneralTools(server) {
    ...
    server.tool("binance_us_ping", "Test connectivity to the Binance.US API. Returns empty object if successful.", {}, async () => {
        try {
    ...
    server.tool("binance_us_server_time", "Get the current server time from Binance.US exchange.", {}, async () => {
        try {
    ...
    server.tool("binance_us_system_status", "Check if Binance.US system is under maintenance. Status 0 = normal, 1 = system maintenance. Requires API key authentication.", {}, async () => {
        try {
    ...
    server.tool("binance_us_exchange_info", "Get current exchange trading rules and trading pair information from Binance.US. Can filter by specific symbol(s) or permissions.", {
