import { z } from "zod";
import { binanceUsRequest, formatKline, formatAggTrade, ORDER_BOOK_VALID_LIMITS, KLINE_INTERVALS, ROLLING_WINDOW_SIZES, MAX_TRADES_LIMIT, MAX_KLINES_LIMIT, BinanceUsApiError, RateLimitError, IpBanError } from "../../config/binanceUsClient.js";
/**
...
export function registerMarketTools(server) {
    ...
    server.tool("binance_us_order_book", "Get order book depth (bids and asks) for a trading pair on Binance.US. Returns price levels with quantities. Weight varies based on limit (1-100: weight 1, 101-500: weight 5, 501-1000: weight 10, 1001-5000: weight 50).", {
    ...
    server.tool("binance_us_recent_trades", "Get recent trades for a trading pair on Binance.US. Returns trade ID, price, quantity, time, and maker/taker info.", {
        symbol: z.string().toUpperCase().describe("Trading pair symbol, e.g., BTCUSD"),
    ...
    server.tool("binance_us_historical_trades", "Get older historical trades for a trading pair. Requires API key with MARKET_DATA permission.", {
        symbol: z.string().toUpperCase().describe("Trading pair symbol, e.g., BTCUSD"),
        limit: z.number().int().min(1).max(MAX_TRADES_LIMIT).optional()
    ...
    server.tool("binance_us_agg_trades", "Get compressed aggregate trades. Trades with same time, order, and price are aggregated.", {
        symbol: z.string().toUpperCase().describe("Trading pair symbol, e.g., BTCUSD"),
        ...
        try {
            ...
            if (limit !== undefined)
                params.limit = limit;
            const result = await binanceUsRequest("GET", "/api/v3/aggTrades", params, false);
    ...
    server.tool("binance_us_klines", "Get Kline/candlestick data for a trading pair. Returns OHLCV data.", {
        symbol: z.string().toUpperCase().describe("Trading pair symbol, e.g., BTCUSD"),
        ...
        try {
            ...
            if (limit !== undefined)
                params.limit = limit;
            const result = await binanceUsRequest("GET", "/api/v3/klines", params, false);
    ...
    server.tool("binance_us_avg_price", "Get current 5-minute rolling weighted average price.", {
        symbol: z.string().toUpperCase().describe("Trading pair symbol, e.g., BTCUSD")
    }, async ({ symbol }) => {
        try {
    ...
    server.tool("binance_us_ticker_24hr", "Get 24-hour rolling window price change statistics.", {
        ...
        try {
            ...
            let summaryText;
            if (isArray) {
                const sorted = [...result].sort((a, b) => parseFloat(b.quoteVolume) - parseFloat(a.quoteVolume));
                const top5 = sorted.slice(0, 5);
                summaryText = `Total symbols: ${result.length}\n` +
                    `Top 5 by volume:\n${top5.map((t) => `  ${t.symbol}: ${t.priceChangePercent}% | Vol: ${parseFloat(t.quoteVolume).toLocaleString()}`).join("\n")}`;
            }
            else {
    ...
    server.tool("binance_us_ticker_price", "Get latest price for symbol(s).", {
        symbol: z.string().toUpperCase().optional().describe("Symbol (e.g., BTCUSD). Cannot use with 'symbols'."),
        symbols: z.array(z.string()).optional().describe("Array of symbols. Cannot use with 'symbol'.")
    ...
    server.tool("binance_us_ticker_book", "Get best bid/ask prices and quantities (top of book).", {
        symbol: z.string().toUpperCase().optional().describe("Symbol (e.g., BTCUSD). Cannot use with 'symbols'."),
        symbols: z.array(z.string()).optional().describe("Array of symbols. Cannot use with 'symbol'.")
    }, async ({ symbol, symbols }) => {
        try {
    ...
    server.tool("binance_us_rolling_window", "Get rolling window price change statistics with custom window sizes (1m to 7d).", {
