 ...
 */
export function registerCustodialSolutionTools(server) {
    // =====================================================================
    // GET /sapi/v1/custodian/balance - Get Account Balance
    // =====================================================================
    server.tool("binance_us_custodial_balance", `Get balance information for Binance.US exchange wallet and custodial sub-account.
...
- lastUpdatedTime: Last update timestamp`, {
    ...
    // =====================================================================
    server.tool("binance_us_custodial_supported_assets", `Get list of assets supported for custodial transfers and settlements.
...
- network: Supported networks for the asset`, {
    ...
    // =====================================================================
    server.tool("binance_us_custodial_wallet_transfer", `Transfer assets from your Binance.US exchange wallet to your custodial sub-account.
...
which can then be traded or settled to your custodial partner.`, {
    ...
    // =====================================================================
    server.tool("binance_us_custodial_custodian_transfer", `Request asset transfer from a custodial partner account to Binance.US custodial sub-account.
...
The actual transfer is executed by the custody partner.`, {
    ...
    // =====================================================================
    server.tool("binance_us_custodial_undo_transfer", `Undo a previous transfer from your custodial partner.
...
This reverses a previous custodian transfer by its transfer ID.`, {
    ...
    // =====================================================================
    server.tool("binance_us_custodial_wallet_transfer_history", `Get history of transfers from Binance.US exchange wallet to custodial sub-account.
...
Returns transfer history with status, amounts, and timestamps.`, {
        ...
        try {
            const params = { rail: rail.toUpperCase() };
            if (transferId)
                params.transferId = transferId;
            if (clientOrderId)
                params.clientOrderId = clientOrderId;
            if (asset)
                params.asset = asset.toUpperCase();
            if (startTime)
    ...
    // =====================================================================
    server.tool("binance_us_custodial_custodian_transfer_history", `Get history of transfers from custodial partner, including ExpressTrade and Undo transfers.
...
Returns transfer history with status, amounts, and timestamps.`, {
        ...
        try {
            ...
            if (clientOrderId)
                params.clientOrderId = clientOrderId;
            if (expressTradeTransfer !== undefined)
                params.expressTradeTransfer = expressTradeTransfer;
            if (asset)
                params.asset = asset.toUpperCase();
            if (startTime)
                params.startTime = startTime;
    ...
    // =====================================================================
    server.tool("binance_us_custodial_settlement", `Request settlement of assets from custodial sub-account to custodial partner.
...
to your custody partner's vault.`, {
    ...
    // =====================================================================
    server.tool("binance_us_custodial_settlement_history", `Get history of settlements from custodial sub-account to custodial partner.
