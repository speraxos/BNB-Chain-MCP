 ...
 */
function getBinanceErrorMessage(code) {
 ...
 */
function validateRecvWindow(recvWindow) {
 ...
 */
export function registerBinanceUsNewOrder(server) {
    server.tool("binance_us_new_order", "Place a new trade order on Binance.US. Supports LIMIT, MARKET, STOP_LOSS, STOP_LOSS_LIMIT, TAKE_PROFIT, TAKE_PROFIT_LIMIT, and LIMIT_MAKER order types.", {
        ...
        try {
            ...
            if (symbolError)
                return { content: [{ type: "text", text: symbolError }], isError: true };
            // Validate recvWindow
 ...
 */
export function registerBinanceUsTestOrder(server) {
    server.tool("binance_us_test_order", "Test a new order on Binance.US without actually placing it. Validates order parameters and signature without executing the trade.", {
        symbol: z.string().describe("Trading pair symbol (e.g., BTCUSD, ETHUSD)"),
        side: OrderSide.describe("Order side: BUY or SELL"),
        type: OrderType.describe("Order type: LIMIT, MARKET, STOP_LOSS, STOP_LOSS_LIMIT, TAKE_PROFIT, TAKE_PROFIT_LIMIT, LIMIT_MAKER"),
 ...
 */
export function registerBinanceUsGetOrder(server) {
    server.tool("binance_us_get_order", "Query the status of a specific order on Binance.US. Either orderId or origClientOrderId must be provided.", {
 ...
 */
export function registerBinanceUsCancelOrder(server) {
    server.tool("binance_us_cancel_order", "Cancel an active order on Binance.US. Either orderId or origClientOrderId must be provided.", {
        symbol: z.string().describe("Trading pair symbol (e.g., BTCUSD)"),
 ...
 */
export function registerBinanceUsCancelReplace(server) {
    server.tool("binance_us_cancel_replace", "Cancel an existing order and place a new order on the same symbol atomically. This is useful for modifying order parameters.", {
        symbol: z.string().describe("Trading pair symbol (e.g., BTCUSD)"),
        side: OrderSide.describe("Order side: BUY or SELL"),
 ...
 */
export function registerBinanceUsOpenOrders(server) {
    server.tool("binance_us_open_orders", "Get all open orders on Binance.US. Can be filtered by symbol. Warning: Querying without symbol is heavier on rate limits (weight 40 vs 3).", {
        ...
        try {
            ...
            const orderList = response.map((order) => `• ${order.symbol} | ${order.side} ${order.type} | ` +
                `Qty: ${order.origQty} @ ${order.price} | ` +
                `Status: ${order.status} | ID: ${order.orderId}`).join('\n');
            return {
                content: [{
                        type: "text",
 ...
 */
export function registerBinanceUsAllOrders(server) {
    server.tool("binance_us_all_orders", "Get all orders (active, canceled, or filled) for a symbol on Binance.US. Returns order history.", {
        symbol: z.string().describe("Trading pair symbol (e.g., BTCUSD)"),
        ...
        try {
            ...
            const orderList = response.slice(0, 10).map((order) => `• ${order.side} ${order.type} | Qty: ${order.origQty} @ ${order.price} | ` +
                `Status: ${order.status} | ID: ${order.orderId} | ${new Date(order.time).toISOString()}`).join('\n');
            return {
                content: [{
                        type: "text",
...
function validateOrderParams(params) {
    ...
    switch (type) {
        case "LIMIT":
            if (!timeInForce)
                return "❌ LIMIT orders require timeInForce parameter.";
            if (!quantity)
                return "❌ LIMIT orders require quantity parameter.";
            if (!price)
                return "❌ LIMIT orders require price parameter.";
            break;
        case "MARKET":
        ...
        case "STOP_LOSS":
            if (!quantity)
                return "❌ STOP_LOSS orders require quantity parameter.";
            if (!stopPrice && !trailingDelta)
                return "❌ STOP_LOSS orders require stopPrice or trailingDelta.";
            break;
        case "STOP_LOSS_LIMIT":
            if (!timeInForce)
                return "❌ STOP_LOSS_LIMIT orders require timeInForce parameter.";
            if (!quantity)
                return "❌ STOP_LOSS_LIMIT orders require quantity parameter.";
            if (!price)
                return "❌ STOP_LOSS_LIMIT orders require price parameter.";
            if (!stopPrice && !trailingDelta)
                return "❌ STOP_LOSS_LIMIT orders require stopPrice or trailingDelta.";
            break;
        case "TAKE_PROFIT":
            if (!quantity)
                return "❌ TAKE_PROFIT orders require quantity parameter.";
            if (!stopPrice && !trailingDelta)
                return "❌ TAKE_PROFIT orders require stopPrice or trailingDelta.";
            break;
        case "TAKE_PROFIT_LIMIT":
            if (!timeInForce)
                return "❌ TAKE_PROFIT_LIMIT orders require timeInForce parameter.";
            if (!quantity)
                return "❌ TAKE_PROFIT_LIMIT orders require quantity parameter.";
            if (!price)
                return "❌ TAKE_PROFIT_LIMIT orders require price parameter.";
            if (!stopPrice && !trailingDelta)
                return "❌ TAKE_PROFIT_LIMIT orders require stopPrice or trailingDelta.";
            break;
        case "LIMIT_MAKER":
            if (!quantity)
                return "❌ LIMIT_MAKER orders require quantity parameter.";
            if (!price)
                return "❌ LIMIT_MAKER orders require price parameter.";
            break;
    }
 ...
 */
export function registerBinanceUsCancelAllOpenOrders(server) {
    server.tool("binance_us_cancel_all_open_orders", "Cancel all active orders on a symbol on Binance.US. This includes OCO orders. Use with caution - this will cancel ALL open orders for the specified symbol.", {
        ...
        try {
            ...
            const cancelledList = response.map((order) => `• Order ID: ${order.orderId} | ${order.side} ${order.type} | ` +
                `Qty: ${order.origQty} @ ${order.price} | Status: ${order.status}`).join('\n');
            return {
                content: [{
                        type: "text",
 ...
 */
export function registerBinanceUsOrderTools(server) {
