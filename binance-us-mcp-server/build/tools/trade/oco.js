 ...
 */
export function registerBinanceUsNewOco(server) {
    server.tool("binance_us_new_oco", "Place a new OCO (One-Cancels-the-Other) order on Binance.US. OCO orders combine a limit order with a stop-loss order. When one triggers, the other is automatically cancelled. Note: For SELL OCOs, limit price > stop price. For BUY OCOs, limit price < stop price.", {
            ...
            return {
                content: [{
                        ...
                        text: `✅ OCO Order placed successfully!\n\n` +
                            ...
                            `\n--- Orders ---\n` +
                            orders.map((order) => `• Order ID: ${order.orderId} | Client ID: ${order.clientOrderId}`).join('\n') +
                            `\n\n--- Order Details ---\n` +
                            orderReports.map((report) => `• ${report.type} | Side: ${report.side} | ` +
                                `Qty: ${report.origQty} @ ${report.price} | ` +
                                `Status: ${report.status}` +
                                (report.stopPrice ? ` | Stop: ${report.stopPrice}` : '')).join('\n') +
 ...
 */
export function registerBinanceUsGetOco(server) {
    server.tool("binance_us_get_oco", "Query a specific OCO order on Binance.US. Either orderListId or origClientOrderId must be provided.", {
            ...
            return {
                content: [{
                        ...
                        text: `📋 OCO Order Details\n\n` +
                            ...
                            `\n--- Orders ---\n` +
                            orders.map((order) => `• Symbol: ${order.symbol} | Order ID: ${order.orderId} | Client ID: ${order.clientOrderId}`).join('\n') +
                            `\n\nFull Response: ${JSON.stringify(response, null, 2)}`
                    }]
            };
        }
 ...
 */
export function registerBinanceUsCancelOco(server) {
    server.tool("binance_us_cancel_oco", "Cancel an entire OCO order on Binance.US. Cancelling any individual leg will cancel the entire OCO.", {
            ...
            return {
                content: [{
                        ...
                        text: `✅ OCO Order cancelled successfully!\n\n` +
                            ...
                            `\n--- Cancelled Orders ---\n` +
                            orderReports.map((report) => `• Order ID: ${report.orderId} | ${report.type} | ` +
                                `Qty: ${report.origQty} @ ${report.price} | ` +
 ...
 */
export function registerBinanceUsOpenOco(server) {
    server.tool("binance_us_open_oco", "Get all open OCO orders on Binance.US.", {
        recvWindow: z.number().optional().describe("Receive window in milliseconds (max 60000)")
    }, async (params) => {
        try {
            // Check API credentials first
            ...
            const ocoList = response.map((oco) => {
 ...
 */
export function registerBinanceUsAllOcoOrders(server) {
    server.tool("binance_us_all_oco_orders", "Get all OCO orders (history) on Binance.US. Returns up to 1000 orders.", {
        ...
        try {
            ...
            const ocoList = response.slice(0, 10).map((oco) => {
                const orders = oco.orders || [];
 ...
 */
export function registerBinanceUsOcoTools(server) {
