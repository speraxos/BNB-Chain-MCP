...
export function registerAccountTools(server) {
    ...
    // GET /api/v3/account
    // =====================================================
    server.tool("binance_us_account_info", "Get current account information including balances and permissions. Returns all asset balances (free and locked), account permissions, and trading status.", {
    ...
    // GET /api/v3/myTrades
    // =====================================================
    server.tool("binance_us_my_trades", "Get trade history for a specific trading pair. Returns executed trades including price, quantity, commission, and timestamps.", {
    ...
    // =====================================================
    server.tool("binance_us_rate_limits", "Get current trade order count rate limits for all time intervals. Shows how many orders you can place within each interval.", {
        recvWindow: z.number().optional().describe("The value cannot be greater than 60000"),
    }, async ({ recvWindow }) => {
        try {
            const params = {};
    ...
    // =====================================================
    server.tool("binance_us_trade_fee", "Get your current maker & taker fee rates for spot trading based on your VIP level. BNB fee discount (25% off) is not factored in.", {
    ...
    // =====================================================
    server.tool("binance_us_trade_volume", "Get total trade volume for the past 30 days. Volume is calculated on a rolling basis every day at 0:00 AM (UTC).", {
        recvWindow: z.number().optional().describe("The value cannot be greater than 60000"),
    }, async ({ recvWindow }) => {
        try {
            const params = {};
